...
impl HarmPattern {
    /// Create new harm pattern
    pub fn new(category: HarmCategory, keywords: Vec<&str>) -> Self {
        Self {
            category,
    ...
    /// Set whether context is required for detection
    pub fn with_context_required(mut self, required: bool) -> Self {
    ...
    pub fn matches(&self, query: &str) -> bool {
        ...
        self.keywords.iter().any(|kw| query_lower.contains(kw))
    }
    
    /// Calculate match confidence
    pub fn match_confidence(&self, query: &str) -> f32 {
        let query_lower = query.to_lowercase();
        ...
        let base_confidence = (matches as f32 / self.keywords.len() as f32).min(1.0);
        base_confidence * self.confidence_weight
    }
}

/// Pattern database for harm detection
pub struct PatternDatabase {
    patterns: Vec<HarmPattern>,
    categories: HashSet<HarmCategory>,
}

impl PatternDatabase {
    /// Create new pattern database with default patterns
    pub fn new() -> Self {
        let mut db = Self {
            patterns: Vec::new(),
    ...
    fn load_default_patterns(&mut self) {
        // Violence patterns
        self.add_pattern(HarmPattern::new(
            HarmCategory::Violence,
            vec!["kill", "murder", "attack", "hurt", "harm", "weapon", "bomb"],
        ).with_weight(0.9));
        
        // Illegal activity patterns
        self.add_pattern(HarmPattern::new(
            HarmCategory::Illegal,
            vec!["hack", "steal", "fraud", "illegal", "crime", "break into"],
        ).with_context_required(true).with_weight(0.8));
        
        // Manipulation patterns
        self.add_pattern(HarmPattern::new(
            HarmCategory::Manipulation,
            vec!["manipulate", "deceive", "trick", "scam", "lie to"],
        ).with_weight(0.7));
        
        // Privacy violation patterns
        self.add_pattern(HarmPattern::new(
            HarmCategory::Privacy,
            vec!["spy on", "track without", "steal data", "unauthorized access"],
        ).with_weight(0.8));
        ...
        self.add_pattern(HarmPattern::new(
            ...
            vec!["exploit", "abuse", "take advantage of"],
        ).with_context_required(true).with_weight(0.7));
        
        // Environmental harm patterns
        self.add_pattern(HarmPattern::new(
            HarmCategory::Environmental,
            vec!["pollute", "destroy nature", "environmental damage"],
        ).with_weight(0.6));
    }
    
    /// Add pattern to database
    pub fn add_pattern(&mut self, pattern: HarmPattern) {
        self.categories.insert(pattern.category);
        self.patterns.push(pattern);
    }
    
    /// Scan query for harmful patterns
    pub fn scan(&self, query: &str) -> Vec<(HarmCategory, f32, Vec<String>)> {
        let mut detections = Vec::new();
        
        for pattern in &self.patterns {
            let confidence = pattern.match_confidence(query);
            if confidence > 0.0 {
                let matched_keywords: Vec<String> = pattern.keywords.iter()
                    .filter(|kw| query.to_lowercase().contains(*kw))
                    .cloned()
                    .collect();
                
                detections.push((pattern.category, confidence, matched_keywords));
            }
        }
    ...
    pub fn categories(&self) -> Vec<HarmCategory> {
...
mod tests {
    ...
    #[test]
    fn test_harm_category_severity() {
        assert_eq!(HarmCategory::Violence.severity(), 5);
        assert_eq!(HarmCategory::SelfHarm.severity(), 5);
        assert!(HarmCategory::Violence.severity() > HarmCategory::Manipulation.severity());
    }
    
    #[test]
    fn test_harm_pattern_matching() {
        let pattern = HarmPattern::new(
            HarmCategory::Violence,
            vec!["kill", "harm"],
        );
        
        assert!(pattern.matches("how to kill"));
        assert!(pattern.matches("will this harm"));
        assert!(!pattern.matches("peaceful solution"));
    }
    ...
    fn test_pattern_confidence() {
        let pattern = HarmPattern::new(
            HarmCategory::Violence,
            vec!["kill", "harm", "attack"],
        ).with_weight(0.9);
        
        let conf1 = pattern.match_confidence("how to kill");
        assert!(conf1 > 0.0);
        
        let conf2 = pattern.match_confidence("peaceful day");
        assert_eq!(conf2, 0.0);
    }
    ...
    fn test_pattern_database() {
        ...
        let detections = db.scan("how to hack into a system");
        assert!(!detections.is_empty());
        
        let (category, confidence, _) = &detections[0];
        assert_eq!(*category, HarmCategory::Illegal);
    ...
    fn test_safe_query() {
        ...
        let detections = db.scan("how to build a web application");
        assert!(detections.is_empty());
    }
}
